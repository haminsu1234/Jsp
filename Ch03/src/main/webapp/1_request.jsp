...
<html>
	<head>
		...
		<!-- 
			...
			request 내장객체
			클라이언트 요청 정보를 갖는 객체
			클라이언트 전송 데이터를 수신하는 기능 제공(getPrameter)
			...
			POST 방식
			전송데이터가 요청객체(request) body에 삽입되어 전송되는방식
			전송데이터 노출 안됨 
	...
	<body>
		...
		<h4> 사용자 입력</h4>
		<form action="./proc/userProc.jsp" method="get"> <!-- requset 객체를 전송 폼에서 정보를 입력하면 aciton의 경로로 데이터 전송 method(전송방식) 보통 get이고 지정해주는게 좋음  -->
		...
		<h4>회원가입</h4>
		<form action="./proc/registerProc.jsp" method="post"><!--전송방식에는 get post가 있음 -->
			<table border="1"><!--  -->
				<tr>
					<td>아이디</td>
