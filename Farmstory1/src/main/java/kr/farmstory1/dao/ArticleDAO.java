...
public class ArticleDAO extends DBHelper {
	...
	public List<ArticleDTO> selectArticles(String cate, int start) {
		...
		try {
			...
			while(rs.next()) {
				...
				dto.setWriter(rs.getString("nick")); //어짜피 list페이지에서 writer를 쓸일이 없기에 writer 자리에 nick값을 넣음
	...
	public void updateUseynComment(ArticleDTO dto) {
		try {
			conn=getConnection();
			psmt=conn.prepareStatement(SQL.UPDATE_USEYN_COMMENT);
			psmt.setInt(1, dto.getNo());
	...
	public List<ArticleDTO> selectNewArticle(String cate, int size){
		List<ArticleDTO> lates = new ArrayList<>();
		try {
			conn=getConnection();
			psmt=conn.prepareStatement(SQL.SELECT_NEW_ARTICLE);
			psmt.setString(1, cate);
			psmt.setInt(2, size);
